...
bool ED_view3d_camera_aim_roll_get(const View3D *v3d, float *r_roll)
{
  if ((v3d->camera == nullptr) || (v3d->camera->type != OB_CAMERA) ||
      (v3d->camera->data == nullptr))
  {
    return false;
  }

  const Camera *camera = reinterpret_cast<const Camera *>(v3d->camera->data);
  *r_roll = camera->aim_roll;
  return true;
}

bool ED_view3d_camera_aim_roll_set(View3D *v3d, const float roll)
{
  if ((v3d->camera == nullptr) || (v3d->camera->type != OB_CAMERA) ||
      (v3d->camera->data == nullptr))
  {
  ...
  const float delta_aim_roll = roll - camera->aim_roll;
  if (delta_aim_roll == 0.0f) {
    return false;
  }

  camera->aim_roll = roll;
